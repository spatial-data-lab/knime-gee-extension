...
3. Before proceeding in KNIME Analytics Platform we have to create the Python environment with all required packages e.g. `geopandas` to do so execute the following command that uses the gee.yml file in the local repository clone: 
   ```bash
   #browse to the folder where the gee.yml file of the local repository clone is located and then execute
   conda env create -f gee.yml 
   
   
   conda activate gee
   
   conda info
   # Record the env location path such as D:\ProgramData\Anaconda3\envs\gee 
   ```
...
6. Go back to the KNIME Analytics Platform and configure the following KNIME Python settings:
   ...
   * KNIME -> File-> Preference> KNIME> Python(Labs) Choose gee environment

7.  Reopen the KNIME Analytics Platform and you should see the gee extension in your KNIME node repository.
