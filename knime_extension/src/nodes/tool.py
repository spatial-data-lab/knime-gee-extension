...
@knext.node(
    name="Reduce Regions (Server-side)",
    node_type=knext.NodeType.MANIPULATOR,
    category=__category,
    icon_path=__NODE_ICON_PATH + "reduceRegions.png",
    after="",
)
@knext.input_port(
    name="GEE Image Connection",
...
@knext.output_table(
    ...
    description="Table containing zonal statistics for each region",
)
class ReduceRegionsServer:
    ...
    limitations. This is essential for large-scale analysis and avoids downloading
    ...
    **Best Practices:**
    ...
    best_effort = knext.BoolParameter(
        "Best Effort",
        "Enable best effort mode for large or complex regions",
        default_value=True,
    )
    ...
    def execute(
        ...
        try:
            ...
            # Remove system properties if present
            ...
            LOGGER.warning(
                f"Successfully calculated zonal statistics for {len(df)} regions using methods: {reduce_methods}"
            )
